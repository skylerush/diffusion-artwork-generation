# EVAL v2 — RESUME script. The original sweep completed: train:full_ft_matched, gen:base (2048),
# gen:lora_r4 (2048), then WEDGED mid gen:lora_r16_s1.0 (WDDM VRAM oversubscription: our fp32
# batch-16 job + a busy desktop shared the 32 GB; the driver paged, throughput fell ~65x, then hung).
#
# This script finishes the job with generation batch 16 -> 8 (halves VRAM pressure; batch size does
# NOT change the generated images, so the comparison stays valid): remaining models + the redo of
# the interrupted one, then the final evaluation.
...
$N     = "2048"
$STEPS = "25"
$BATCH = "8"      # was 16 — reduced after the VRAM-oversubscription wedge
...
Log "=================== EVAL v2 RESUME (batch $BATCH) ==================="
